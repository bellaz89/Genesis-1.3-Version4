...
#define TEST_N_VALUES 100000

double Inverfc_value(double y)
{
...
TEST(compat, erfc_inv){

    for(unsigned int i = 1; i<=TEST_N_VALUES; i++){

        double vold = Inverfc_value(2.0*static_cast<double>(i)/static_cast<double>(TEST_N_VALUES+1));
        double vnew = erfc_inv(2.0*static_cast<double>(i)/static_cast<double>(TEST_N_VALUES+1));
