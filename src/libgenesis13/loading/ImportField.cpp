...
#include <libgenesis13/io/ReadFieldHDF5.h>
...
bool ImportField::init(int rank, int size, map<string, string>* arg,
    ...
    if (arg->find("time")!=end)    {
        dotime = atob(arg->at("time").c_str());
        arg->erase(arg->find("time"));
    }
    if (arg->find("harmonic")!=end) {
        harm = atoi(arg->at("harmonic").c_str());
        arg->erase(arg->find("harmonic"));
    }
    if (arg->size()!=0) {
        if (rank==0) {
            cout << "*** Error: Unknown elements in &importfield" << endl;
            this->usage();
        }
        return false;
    }
    ...
    if (idx<0) {
        field=new Field;
        if (rank==0) {
            cout << "Importing radiation field distribution from file: " << file << " ..." <<
                 endl;
        }
    } else {
        if (rank==0) {
            cout << "*** Error: Cannot import field, because field is already defined" << endl;
        }
        return false;
    }
    field->init(time->getNodeNSlice(), import.getNGrid(), import.getDGrid(), lambda,
                sample*lambda, s[0], harm);
    if (idx<0) {
