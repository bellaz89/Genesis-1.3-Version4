...
bool LoadBeam::init(int rank, int size, map<string, string>* arg, Beam* beam,
                    Setup* setup, Time* time, Profile* prof, Lattice* lat) {
    if (beam->beam.size()>0) {
        if (rank==0) {
            cout << "*** Error: Cannot generat beam, because beam is already defined" << endl;
        }
        return false;
    }
    ...
    if (arg->find("gamma")!=end    ) {
        this->reference(arg->at("gamma"), &gamma, &gammaref);
        arg->erase(arg->find("gamma"));
    }
    if (arg->find("delgam")!=end   ) {
        this->reference(arg->at("delgam"), &delgam, &delgamref);
        arg->erase(arg->find("delgam"));
    }
    if (arg->find("current")!=end  ) {
        this->reference(arg->at("current"), &current, &currentref);
        arg->erase(arg->find("current"));
    }
    if (arg->find("ex")!=end       ) {
        this->reference(arg->at("ex"), &ex, &exref);
        arg->erase(arg->find("ex"));
    }
    if (arg->find("ey")!=end       ) {
        this->reference(arg->at("ey"), &ey, &eyref);
        arg->erase(arg->find("ey"));
    }
    if (arg->find("betax")!=end    ) {
        this->reference(arg->at("betax"), &betax, &betaxref);
        arg->erase(arg->find("betax"));
    }
    if (arg->find("betay")!=end    ) {
        this->reference(arg->at("betay"), &betay, &betayref);
        arg->erase(arg->find("betay"));
    }
    if (arg->find("alphax")!=end   ) {
        this->reference(arg->at("alphax"), &alphax, &alphaxref);
        arg->erase(arg->find("alphax"));
    }
    if (arg->find("alphay")!=end   ) {
        this->reference(arg->at("alphay"), &alphay, &alphayref);
        arg->erase(arg->find("alphay"));
    }
    if (arg->find("xcenter")!=end  ) {
        this->reference(arg->at("xcenter"), &xcen, &xcenref);
        arg->erase(arg->find("xcenter"));
    }
    if (arg->find("ycenter")!=end  ) {
        this->reference(arg->at("ycenter"), &ycen, &ycenref);
        arg->erase(arg->find("ycenter"));
    }
    if (arg->find("pxcenter")!=end ) {
        this->reference(arg->at("pxcenter"), &pxcen, &pxcenref);
        arg->erase(arg->find("pxcenter"));
    }
    if (arg->find("pycenter")!=end ) {
        this->reference(arg->at("pycenter"), &pycen, &pycenref);
        arg->erase(arg->find("pycenter"));
    }
    if (arg->find("bunch")!=end    ) {
        this->reference(arg->at("bunch"), &bunch, &bunchref);
        arg->erase(arg->find("bunch"));
    }
    if (arg->find("bunchphase")!=end ) {
        this->reference(arg->at("bunchphase"), &bunchphase, &bunchphaseref);
        arg->erase(arg->find("bunchphase"));
    }
    if (arg->find("emod")!=end     ) {
        this->reference(arg->at("emod"), &emod, &emodref);
        arg->erase(arg->find("emod"));
    }
    if (arg->find("emodphase")!=end ) {
        this->reference(arg->at("emodphase"), &emodphase, &emodphaseref);
        arg->erase(arg->find("emodphase"));
    }
    if (arg->size()!=0) {
        if (rank==0) {
            cout << "*** Error: Unknown elements in &beam" << endl;
            this->usage();
        }
        return false;
    }
    ...
    for (int j=0; j<time->getNodeNSlice(); j++) {
        ...
        int npartloc=npart;
        if (one4one) {
            npartloc=static_cast<int>(round(ne));
        }
        if (npartloc>nbeam) {
            delete [] beamslice;
            nbeam=npartloc;
