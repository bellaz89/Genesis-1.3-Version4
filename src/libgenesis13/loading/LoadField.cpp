...
bool LoadField::init(int rank, int size, map<string, string>* arg,
    ...
    if (arg->find("lambda")!=end) {
        this->reference(arg->at("lambda"), &lambda, &lambdaref);
        arg->erase(arg->find("lambda"));
    }
    if (arg->find("power")!=end) {
        this->reference(arg->at("power"), &power, &powerref);
        arg->erase(arg->find("power"));
    }
    if (arg->find("phase")!=end) {
        this->reference(arg->at("phase"), &phase, &phaseref);
        arg->erase(arg->find("phase"));
    }
    if (arg->find("waist_pos")!=end) {
        this->reference(arg->at("waist_pos"), &z0, &z0ref);
        arg->erase(arg->find("waist_pos"));
    }
    if (arg->find("waist_size")!=end) {
        this->reference(arg->at("waist_size"), &w0, &w0ref);
        arg->erase(arg->find("waist_size"));
    }
    if (arg->find("xcenter")!=end) {
        xcen = atof(arg->at("xcenter").c_str());
        arg->erase(arg->find("xcenter"));
    }
    if (arg->find("ycenter")!=end) {
        ycen = atof(arg->at("ycenter").c_str());
        arg->erase(arg->find("ycenter"));
    }
    if (arg->find("xangle")!=end)  {
        xangle = atof(arg->at("xangle").c_str());
        arg->erase(arg->find("xangle"));
    }
    if (arg->find("yangle")!=end)  {
        yangle = atof(arg->at("yangle").c_str());
        arg->erase(arg->find("yangle"));
    }
    if (arg->find("dgrid")!=end) {
        dgrid = atof(arg->at("dgrid").c_str());
        arg->erase(arg->find("dgrid"));
    }
    if (arg->find("ngrid")!=end) {
        ngrid = atoi(arg->at("ngrid").c_str());
        arg->erase(arg->find("ngrid"));
    }
    if (arg->find("harm")!=end)  {
        harm  = atoi(arg->at("harm").c_str());
        arg->erase(arg->find("harm"));
    }
    if (arg->find("nx")!=end)    {
        nx  = atoi(arg->at("nx").c_str());
        arg->erase(arg->find("nx"));
    }
    if (arg->find("ny")!=end)    {
        ny  = atoi(arg->at("ny").c_str());
        arg->erase(arg->find("ny"));
    }
    if (arg->find("accumulate")!=end) {
        add  = atob(arg->at("accumulate"));
        arg->erase(arg->find("accumulate"));
    }
    if (arg->size()!=0) {
        if (rank==0) {
            cout << "*** Error: Unknown elements in &field" << endl;
            this->usage();
        }
        return false;
    }
