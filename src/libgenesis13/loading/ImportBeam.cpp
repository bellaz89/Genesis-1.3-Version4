...
bool ImportBeam::init(int rank, int size, map<string, string>* arg, Beam* beam,
                      Setup* setup, Time* time) {
    if (beam->beam.size()>0) {
        if (rank==0) {
            cout << "*** Error: Cannot import beam, because beam is already defined" << endl;
        }
        return false;
    }
    ...
    if (arg->size()!=0) {
        if (rank==0) {
            cout << "*** Error: Unknown elements in &importbeam" << endl;
            this->usage();
        }
        return false;
    }
