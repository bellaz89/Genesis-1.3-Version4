...
public:
    ...
    double reflength,
           slicelength;  // for conversion of theta in Particle to real position
...
inline void Beam::initEField(double rmax, int ngrid, int nz, int nphi,
                             double lambda) {
    solver.initEField(rmax, ngrid, nz, nphi, lambda);
    return;
}

inline void Beam::initWake(unsigned int ns, double ds, double* wakeres,
                           double ztrans, bool transient) {
