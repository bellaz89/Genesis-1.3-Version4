...
bool Lattice::generateLattice(double delz, double lambda, double gamma,
                              AlterLattice* alt, Undulator* und) {
    ...
    for (int i=0; i<ndata; i++) {
        ...
        und->chic_lt[i]=
            lat_lt[i];       // here it is the total length but it will change to the time delay.
...
void Lattice::calcSlippage(double lambda, double gamma) {
    int nz=lat_aw.size();
    lat_slip.resize(
        nz);                                  // this needs to be improved for chicanes
    lat_phase.resize(nz);
...
void Lattice::getMatchedOptics(double* betax, double* alphax, double* betay,
                               double* alphay) {
    if (matched) {
        *betax =mbetax;
        *alphax=malphax;
...
void Lattice::match(int rank, double z0, double gammaref) {
    ...
    if ((i==lat_z.size())&&((lat_z[i-1]+lat_dz[i-1])<z0)) {
        if (rank==0) {
            cout << "*** Warning: Matching position beyond end of lattice. Ignoring matching command"
                 << endl;
        }
        matched=false;
        return;
    ...
    if (matched=opt.match(&mbetax, &malphax, &mbetay, &malphay, &phix, &phiy)) {
        ...
        if (rank==0) {
            cout << "*** Warning: Cannot find a matching solution" << endl;
        }
        matched=false;
    }
    return;
        ...
        if (iele!=-1) {
            if (iele!=lastChicane) {
                if ((!inUnd)&&(!inQuad)) {
                    ...
                    lat_ld[i]=chicane->ld;
                    lat_lt[i]=
                        chicane->l;    // save length of chicane in case that the chicane is split in parts
                    lastChicane=iele;
                } else {
                    cout << "*** Warning: Chicane inside Undulator or Quadrupole. Ignoring Chicane" <<
                         endl;
                }
            }
        }
