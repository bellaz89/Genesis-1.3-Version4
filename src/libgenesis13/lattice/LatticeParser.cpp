...
bool LatticeParser::parse(string file, string line, int rank, vector<Element*> &lat,
    ...
    if (streaming) {
        ...
        if (!fin) {
            if (rank==0) {
                cout << "*** Error: Cannot open magnetic lattice file: " << file << endl;
            }
            return false;
        }
    ...
    while(getline(input, instring)) {  // read line
        //    cout << instring<<endl;
        this->trim(instring);
        if ((!instring.compare(0, 1, "#") || instring.length() < 1)) {
            continue;    // skip comment and empty rows
        }
        comstring.append(" ");
    ...
    size_t pos;
    while ((pos=comstring.find_first_of(";")) !=
            string::npos) { // split into individual lines
    ...
    if ((idx>-1) && (type[idx].compare("line")==0)) {
        ...
        if (rank==0) {
            cout << "*** Error: Lattice file does ";
            cout << "not contain the beamline: " << line << endl;
        }
        return false;
    }
    if (error==false) {
    ...
    for (int i=0; i<sequence.size(); i++) {
        ...
        if (type[idx].compare("quad")==0) {
            error=false;
            lat.push_back(this->parseQuad(idx, rank, z));
        }
        if (type[idx].compare("undu")==0) {
            error=false;
            lat.push_back(this->parseID(idx, rank, z));
        }
        if (type[idx].compare("drif")==0) {
            error=false;
            lat.push_back(this->parseDrift(idx, rank, z));
        }
        if (type[idx].compare("corr")==0) {
            error=false;
            lat.push_back(this->parseCorrector(idx, rank, z));
        }
        if (type[idx].compare("chic")==0) {
            error=false;
            lat.push_back(this->parseChicane(idx, rank, z));
        }
        if (type[idx].compare("mark")==0) {
            error=false;
            lat.push_back(this->parseMarker(idx, rank, z));
        }
        if (type[idx].compare("phas")==0) {
            error=false;
            lat.push_back(this->parsePhaseshifter(idx, rank, z));
        }
        if (error) {
            if (rank==0) {
                cout << "*** Error: Unknown element type " << type[idx];
                cout << " in lattice" << endl;
            }
            return false;
        }
    }
...
ID* LatticeParser::parseID(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (pos==string::npos) {
            if (rank==0) {
                cout << "*** Warning: Ignoring invalid format: " << par[i] << " for element " <<
                     label[idx]<< endl;
            }
            continue;
        }
        ...
        bool found=false;
        if (fld.compare("l")==0)    {
            ele->l=atof(val.c_str());
            found=true;
        };
        if (fld.compare("lambdau")==0) {
            ele->lambdau=atof(val.c_str());
            found=true;
        };
        if (fld.compare("aw")==0)   {
            ele->aw=atof(val.c_str());
            found=true;
        };
        if (fld.compare("aw_perp")==0)  {
            ele->paw=atof(val.c_str());
            found=true;
        };
        if (fld.compare("nwig")==0) {
            ele->nwig=atof(val.c_str());
            found=true;
        };
        if (fld.compare("kx")==0)   {
            ele->kx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("ky")==0)   {
            ele->ky=atof(val.c_str());
            found=true;
        };
        if (fld.compare("kx_perp")==0)  {
            ele->pkx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("ky_perp")==0)  {
            ele->pky=atof(val.c_str());
            found=true;
        };
        if (fld.compare("ax")==0)   {
            ele->ax=atof(val.c_str());
            found=true;
        };
        if (fld.compare("ay")==0)   {
            ele->ay=atof(val.c_str());
            found=true;
        };
        if (fld.compare("gradx")==0) {
            ele->gradx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("grady")==0) {
            ele->grady=atof(val.c_str());
            found=true;
        };
        if (fld.compare("gradx_perp")==0) {
            ele->pdadx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("grady_perp")==0) {
            ele->pdady=atof(val.c_str());
            found=true;
        };
        if (fld.compare("phase_perp")==0) {
            ele->phase=atof(val.c_str());
            found=true;
        };
        if (fld.compare("helical")==0)   {
            ele->helical=atob(val.c_str());
            found=true;
        };
        if (found==false) {
            if (rank==0) {
                cout << "*** Warning: Ignoring unknow parameter: " << fld << " for element " <<
                     label[idx]<< endl;
            }
        }
    }
...
Corrector* LatticeParser::parseCorrector(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (fld.compare("cx")==0)   {
            ele->cx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("cy")==0)   {
            ele->cy=atof(val.c_str());
            found=true;
        };
        if (found==false) {
            if (rank==0) {
...
Chicane* LatticeParser::parseChicane(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (fld.compare("delay")==0) {
            ele->delay=atof(val.c_str());
            found=true;
        };
        if (fld.compare("lb")==0)   {
            ele->lb=atof(val.c_str());
            found=true;
        };
        if (fld.compare("ld")==0)   {
            ele->ld=atof(val.c_str());
            found=true;
        };
        if (found==false) {
            if (rank==0) {
...
Marker* LatticeParser::parseMarker(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (fld.compare("dumpfield")==0) {
            ele->action|=1*tag;
            found=true;
        }
        if (fld.compare("dumpbeam")==0) {
            ele->action|=2*tag;
            found=true;
        }
        if (fld.compare("sort")==0) {
            ele->action|=4*tag;
            found=true;
        }
        if (fld.compare("stop")==0) {
            ele->action|=8*tag;
            found=true;
        }
        if (found==false) {
...
Quadrupole* LatticeParser::parseQuad(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (fld.compare("k1")==0) {
            ele->k1=atof(val.c_str());
            found=true;
        };
        if (fld.compare("dx")==0) {
            ele->dx=atof(val.c_str());
            found=true;
        };
        if (fld.compare("dy")==0) {
            ele->dy=atof(val.c_str());
            found=true;
        };
        if (found==false) {
            if (rank==0) {
...
Phaseshifter* LatticeParser::parsePhaseshifter(int idx, int rank, double zin) {
    ...
    for (int i=0; i<par.size(); i++) {
        ...
        if (fld.compare("phi")==0) {
            ele->phi=atof(val.c_str());
            found=true;
        };
        if (found==false) {
            if (rank==0) {
...
bool LatticeParser::resolve(int idx, int recursion, int rank) {
    if (recursion < 0) {
        if (rank==0) {
            cout << "*** Error: Too many nested references" << endl;
        }
        return false;
    }
    ...
    if (iref<0) {
        if (rank==0) {
            cout << "*** Error: Unresolved reference: " << ref <<" for element: " << label[idx]
                 << endl;
        }
        return false;
    }
    if (this->resolve(iref, recursion-1, rank)==false) {
        return false;
    }
...
bool LatticeParser::unroll(int idx, int recursion, int rank) {
    if (recursion < 0) {
        if (rank==0) {
            cout << "*** Error: Too many nested elements in selected beamline" << endl;
        }
        return false;
    }
    vector<string> line;
    ...
    for (int i=0; i<line.size(); i++) {
        ...
        if (ix <0) {
            if (rank==0) {
                cout << "*** Error: Undefined element in beamline: " << line[i] << endl;
            }
            return false;
        }
