...
bool Parser::open(string file, int inrank, bool streaming) {
    ...
    if (!streaming) {
        ...
        if (!fin) {
            if (rank==0) {
                cout << "*** Error: Cannot open main input file: " << file << endl;
            }
            return false;
        }
...
bool Parser::parse(string* element, map<string, string>* argument) {
    ...
    while(getline(input, instring)) {  // read line
        ...
        if ((instring.compare("&end")==0) ||
                (instring.compare("&END")==0) ||
                (instring.compare("&End")==0)) {
            if (!accumulate) {
                if (rank==0) {
                    cout << "*** Error: Termination string outside element";
                    cout<< " definition in input file" << endl;
                }
                return false;
            }
        ...
        if (!instring.compare(0, 1, "&")) {
            if (accumulate) {
                if (rank==0) {
                    cout << "*** Error: Nested elements in main input file" << endl;
                }
                return false;
            }
            accumulate=true;
...
bool Parser::fillMap(string* list, map<string, string>* map) {
    ...
    while ((pos=list->find_first_of(";")) !=
        ...
        if (val.length()>0) {
            ...
            if (tpos ==string::npos) {
                if (rank==0) {
                    cout << "*** Error: Invalid format " << val << " in input file" << endl;
                }
                return false;
            } else {
