...
bool Time::init(int inrank, int insize, map<string,string> *arg, Setup *setup)
{
    ...
    if (arg->find("sample")!=end) {
        sample = atoi(arg->at("sample").c_str());
        arg->erase(arg->find("sample"));
    }
    ...
    if (arg->size()!=0){
        if (rank==0){ 
        cout << "*** Error: Unknown elements in &time" << endl; this->usage();
        }
        return false;
    }
...
void Time::finishInit(Setup *setup)
{
    ...
    if (rank==0){
        cout << "Setting up time window of " << slen*1e6 << " microns with ";
        cout << nslice <<" sample points..." << endl;
    }
}
