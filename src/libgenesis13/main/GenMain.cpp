...
double genmain (string mainstring, string latstring, bool streaming,
    ...
    if (rank==0) {
        time(&timer);
        cout << "---------------------------------------------" << endl;
        cout << "GENESIS - Version " << VERSIONMAJOR <<"."<< VERSIONMINOR << "." <<
             VERSIONREVISION ;
        if (VERSIONBETA) {
            cout << " (beta)";
        }
    ...
    vector<Field* >
    field;   // an vector of various field components (harmonics, vertical/horizonthal components)
    Beam* beam =new Beam;
    //----------------------------------------------------------
    ...
    while(parser.parse(&element, &argument)) {
        ...
        if (element.compare("&setup")==0) {
            if (!setup->init(rank, &argument, lattice, latstring, streaming)) {
                break;
            }
        ...
        if (element.compare("&alter_setup")==0) {
            ...
            if (!altersetup->init(rank, &argument, setup, lattice, timewindow, beam, &field)) {
                break;
            }
            delete altersetup;
            continue;
        ...
        if (element.compare("&lattice")==0) {
            if (!alt->init(rank, size, &argument, lattice, setup)) {
                break;
            }
            continue;
        }
        ...
        if ((element.compare("&profile_const")==0)||
            ...
            if (!profile->init(rank, &argument, element)) {
                break;
            }
            continue;
        }
        ...
        if (element.compare("&time")==0) {
            if (!timewindow->init(rank, size, &argument, setup)) {
                break;
            }
            continue;
        }
        ...
        if (element.compare("&field")==0) {
            ...
            if (!loadfield->init(rank, size, &argument, &field, setup, timewindow, profile)) {
                break;
            }
            delete loadfield;
            continue;
        ...
        if (element.compare("&efield")==0) {
            ...
            if (!efield->init(rank, size, &argument, beam, setup, timewindow)) {
                break;
            }
            delete efield;
            continue;
        ...
        if (element.compare("&sponrad")==0) {
            ...
            if (!sponrad->init(rank, size, &argument, beam)) {
                break;
            }
            delete sponrad;
            continue;
        ...
        if (element.compare("&wake")==0) {
            Wake* wake = new Wake;
            if (!wake->init(rank, size, &argument, timewindow, setup, beam)) {
                break;
            }
            delete wake;
            continue;
        ...
        if (element.compare("&beam")==0) {
            ...
            if (!loadbeam->init(rank, size, &argument, beam, setup, timewindow, profile,
                                lattice)) {
                break;
            }
            delete loadbeam;
        ...
        if (element.compare("&importdistribution")==0) {
            ...
            if (!sddsbeam->init(rank, size, &argument, beam, setup, timewindow, lattice)) {
                break;
            }
            delete sddsbeam;
            continue;
        ...
        if (element.compare("&track")==0) {
            ...
            if (!track->init(rank, size, &argument, beam, &field, setup, lattice, alt,
                             timewindow, supressOutput)) {
                break;
            }
            delete track;
            continue;
        ...
        if (element.compare("&write")==0) {
            Dump* dump=new Dump;
            if (!dump->init(rank, size, &argument, setup, beam, &field)) {
                break;
            }
            delete dump;
            continue;
        ...
        if (element.compare("&importbeam")==0) {
            ...
            if (!import->init(rank, size, &argument, beam, setup, timewindow)) {
                break;
            }
            delete import;
            continue;
        ...
        if (element.compare("&importfield")==0) {
            ...
            if (!import->init(rank, size, &argument, &field, setup, timewindow)) {
                break;
            }
            delete import;
            continue;
