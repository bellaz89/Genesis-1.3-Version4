...
bool EField::init(int rank, int size, map<string,string> *arg,  Beam *beam, Setup *setup, Time *time)
{
    ...
    if (arg->find("rmax")!=end) {
        rmax  = atof(arg->at("rmax").c_str());
        arg->erase(arg->find("rmax"));
    }
    ...
    if (arg->find("nz")!=end) {nz = atoi(arg->at("nz").c_str());
        arg->erase(arg->find("nz"));
    }

    if (arg->find("nphi")!=end) {nphi = atoi(arg->at("nphi").c_str());
        arg->erase(arg->find("nphi"));
    }

    if (arg->size()!=0){
        if (rank==0){ 
            cout << "*** Error: Unknown elements in &efield" << endl; 
            this->usage();
        }
        return false;
    }
