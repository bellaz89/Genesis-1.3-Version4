...
void WriteBeamHDF5::write(string fileroot, Beam* beam) {
    ...
    if (rank == 0) {
        cout << "Writing particle distribution to file: ";
        cout << filename << " ..." << endl;
    }
    ...
    for (int i=0; i<(ntotal); i++) {
        ...
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).gamma;
            }
        }
        this->writeSingleNode(gid, "gamma", &work);
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).theta;
            }
        }
        this->writeSingleNode(gid, "theta", &work);
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).x;
            }
        }
        this->writeSingleNode(gid, "x", &work);
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).y;
            }
        }
        this->writeSingleNode(gid, "y", &work);
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).px;
            }
        }
        this->writeSingleNode(gid, "px", &work);
        if (s0==0) {
            for (int ip=0; ip<npart; ip++) {
                work[ip]=beam->beam.at(islice).at(ip).py;
            }
        }
