...
bool ReadBeamHDF5::readGlobal(int rank, int size, string file, Setup* setup,
                              Time* time, bool dotime) {
    isOpen=false;
    // read global data
    double reflen;
    int nbins, one4one;
    ...
    if (fabs((lambda-reflen)/lambda)>1e-6) {
        if (rank==0) {
            cout << "*** Error: Mismatch between reference length ";
            cout << "of run and of input file" << endl;
        }
        return false;
    }
    if (nbins!=setup->getNbins()) {
        if (rank==0) {
            cout << "*** Error: Mismatch between beamlet size (NBINS) ";
            cout << "of run and of input file" << endl;
        }
        return false;
    }
    if (((setup->getOne4One())&&(one4one==0)) || ((!setup->getOne4One())
            &&(one4one!=0))) {
        if (rank==0) {
            cout << "*** Error: Mismatch between ONE4ONE ";
            cout << "flag of run and of input file" << endl;
        }
        return false;
    }
    ...
    if (fabs(slicelen-lambda*sample)/slicelen>1e-6) {
        if (rank==0) {
            cout << "*** Error: Mismatch in sample rate of run and of input file" << endl;
        }
        return false;
    }
    return true;
...
bool ReadBeamHDF5::readSlice(double s, vector<Particle>* slice, double* current,
    ...
    *current=0;
    if(!isOpen) {
        return false;    // skip if partfile option is not selected
    }
    double rslice=(s-s0)/slicelen;
    if (fabs(rslice-round(rslice))>1e-3) {
        return false;
    }
    ...
    if ((islice<1) || (islice> count)) {
        if (one4one) {
            return false;    // one4one do nothing
        }
