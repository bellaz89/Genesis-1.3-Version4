...
bool SDDSBeam::init(int inrank, int insize, map<string, string>* arg, Beam* beam,
    ...
    if ( (npart % nbins) != 0) {
        if (rank==0) {
            cout << "*** Error: NPART is not a multiple of NBINS" << endl;
        }
        return false;
    }
    ...
    if (arg->find("charge")!=end)     {
        charge = atof(arg->at("charge").c_str());
        arg->erase(arg->find("charge"));
    }
    if (arg->find("slicewidth")!=end) {
        ds     = atof(arg->at("slicewidth").c_str());
        arg->erase(arg->find("slicewidth"));
    }
    if (arg->find("match_start")!=end)    {
        matchs0    = atof(arg->at("match_start").c_str());
        arg->erase(arg->find("match_start"));
    }
    if (arg->find("match_end")!=end)      {
        matchs1    = atof(arg->at("match_end").c_str());
        arg->erase(arg->find("match_end"));
    }
    if (arg->find("align_start")!=end)    {
        aligns0    = atof(arg->at("align_start").c_str());
        arg->erase(arg->find("align_start"));
    }
    if (arg->find("align_end")!=end)      {
        aligns1    = atof(arg->at("align_end").c_str());
        arg->erase(arg->find("align_end"));
    }
    if (arg->find("betax")!=end)    {
        betax = atof(arg->at("betax").c_str());
        arg->erase(arg->find("betax"));
    }
    if (arg->find("betay")!=end)    {
        betay = atof(arg->at("betay").c_str());
        arg->erase(arg->find("betay"));
    }
    if (arg->find("alphax")!=end)   {
        alphax= atof(arg->at("alphax").c_str());
        arg->erase(arg->find("alphax"));
    }
    if (arg->find("alphay")!=end)   {
        alphay= atof(arg->at("alphay").c_str());
        arg->erase(arg->find("alphay"));
    }
    if (arg->find("x0")!=end)       {
        xcen  = atof(arg->at("x0").c_str());
        arg->erase(arg->find("x0"));
    }
    if (arg->find("y0")!=end)       {
        ycen  = atof(arg->at("y0").c_str());
        arg->erase(arg->find("y0"));
    }
    if (arg->find("px0")!=end)      {
        pxcen = atof(arg->at("px0").c_str());
        arg->erase(arg->find("px0"));
    }
    if (arg->find("py0")!=end)      {
        pycen = atof(arg->at("py0").c_str());
        arg->erase(arg->find("py0"));
    }
    if (arg->find("gamma0")!=end)   {
        gamma = atof(arg->at("gamma0").c_str());
        arg->erase(arg->find("gamma0"));
    }
    if (arg->find("align")!=end)    {
        align = atoi(arg->at("align").c_str());
        arg->erase(arg->find("align"));
    }
    if (arg->find("match")!=end)    {
        match = atob(arg->at("match").c_str());
        arg->erase(arg->find("match"));
    }
    if (arg->find("center")!=end)   {
        center= atob(arg->at("center").c_str());
        arg->erase(arg->find("center"));
    }
    if (arg->find("settimewindow")!=end)   {
        settime= atob(arg->at("settimewindow").c_str());
        arg->erase(arg->find("settimewindow"));
    }
    if (arg->size()!=0) {
        if (rank==0) {
            cout << "*** Error: Unknown elements in &importdistribution" << endl;
            this->usage();
        }
        return false;
    }
    ...
    if (rank==0) {
        cout << "Charge of external distribution: " << charge << endl;
    }
    ...
    if (rank==0) {
        cout << "Particles in external distribution: " << ntotal << endl;
    }
    ...
    int nsize=nchunk;
    if ((rank*nchunk+nsize)>ntotal) {
        nsize=ntotal-rank*nchunk;
    }
    t.resize(nsize);
    g.resize(nsize);
    x.resize(nsize);
    ...
    if (match) {
        if (rank==0) {
            cout << "Matching external distribution..." << endl;
        }
        for (int i=0; i<nsize; i++) {
            if (!center) {
                gamma=-gavg;
    ...
    if (center) {
        if (rank==0) {
            cout << "Centering external distribution..." << endl;
        }
        gavg=gamma-gavg;
        xavg=xcen-xavg;
        yavg=ycen-yavg;
    ...
    if ((match)||(center)) {
        if (rank==0) {
            cout << "Reanalysing matched and aligned distribution..." << endl;
        }
        this->analyse(ttotal, nsize);
    }
    ...
    if (rank==0) {
        cout << "Sorting external distribution..." << endl;
    }
    double dslen=
        ds*ttotal;  // ds is the relative width to extract the samples (equivalent to 1/NDCUT)
    ...
    if (rank==0) {
        cout << "Generating internal particle distribution..." << endl;
    }
...
void SDDSBeam::addParticles(vector<Particle>* beam, int mpart) {
    ...
    while (ndist<mpart) {
        ...
        par.px=0.5*(beam->at(n1).px+beam->at(n2).px)+(2*ran->getElement()-1)*(beam->at(
                    n1).px-beam->at(n2).px);
        par.y =0.5*(beam->at(n1).y +beam->at(n2).y) +(2*ran->getElement()-1)*(beam->at(
                    n1).y -beam->at(n2).y);
        par.py=0.5*(beam->at(n1).py+beam->at(n2).py)+(2*ran->getElement()-1)*(beam->at(
                    n1).py-beam->at(n2).py);
        beam->push_back(par);
        ndist++;
    }
