...
void Output::close() {
    ...
    if (norphans > 1) { /* expect 1 for the file we have not closed */
        ...
        for (i=0; i<norphans; i++) {
            ...
            cout << "Slice " << s0/ds << " : " << i+1 << " of " << norphans;
            cout << " things still open: " << objects[i] << " with name " << name;
            cout << " of type " << info.type << endl;
        }
    }
    H5Fclose(fid);
}
...
void Output::writeGlobal(double gamma, double lambda, double sample, double slen,
                         bool one4one, bool time, bool scan) {
    this->writeMeta();
    vector<double> tmp;
    tmp.resize(1);
    hid_t gid;
